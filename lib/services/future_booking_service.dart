...
class FutureBookingService {
  ...
  static Future<bool> clientCancelBooking({
    ...
    try {
      ...
      // Best-effort refund for card payments (creates refund_request for admin).
      () async {
        try {
          await RefundService.refundFutureBooking(
            bookingId: id,
            reason: 'client_cancelled:$reason',
            initiatedBy: userId,
          );
  ...
  static Future<String?> _createTasksManagementRequestForFutureBooking({
    ...
    try {
      ...
      // Skip inference for RFQ bookings — they use the AI quote total, not
      // catalog prices, so inferring a catalog task would set a wrong cost.
      if (effectiveJobIds.isEmpty && rfqTotal <= 0) {
        final inferred =
      ...
      String effectiveAddress = trimmedProvided.isNotEmpty
          ? trimmedProvided
          : (serviceOnCurrentLocation ? 'Client current location' : 'N/A');

      // When the client selected "current location" and no real address was
      // supplied, reverse-geocode the GPS coordinates so the artisan sees an
      // actual street address instead of "Client current location".
      if (serviceOnCurrentLocation &&
          (effectiveAddress == 'Client current location' ||
           effectiveAddress.toLowerCase() == 'current location')) {
        try {
          final dLat = double.tryParse(userLat);
          final dLng = double.tryParse(userLng);
          if (dLat != null && dLng != null && dLat != 0 && dLng != 0) {
            final placemarks = await placemarkFromCoordinates(dLat, dLng);
          ...
          debugPrint('Reverse geocode failed, keeping default address: $e');
        }
      }
      ...
      // For RFQ bookings, use the AI quote total directly instead of
      // resolving prices from the tasks catalog (which would give a wrong amount).
      ...
      if (rfqTotal > 0) {
        ...
        totalCost = resolvedTaskCosts.values.fold<double>(
          0.0,
          (sum, cost) => sum + cost,
        );
      }
  ...
  static Future<Map<String, dynamic>> createBookingAndNotify({
    ...
    if (!isRFQ) {
      ...
      if ((assignedArtisanId == null || assignedArtisanId.trim().isEmpty) &&
          (createdBy.trim().toLowerCase() == 'voice_ai' ||
           ...
           createdBy.trim().toLowerCase() == 'manual')) {
        try {
    ...
    // If no artisan found, try one more time with RELAXED criteria - just find
    // ANY active published artisan in that category, ignoring tasks and schedules
    // for now (admin can reassign if needed)
    if (assignedArtisanId == null &&
      ...
      try {
        final safeCatName = (categoryName ?? '').trim().toLowerCase();
        final safeCatId = (categoryId ?? '').trim().toLowerCase();

        QuerySnapshot<Map<String, dynamic>> allArtisans;
        final query = serviceProviderRef.where('status', isEqualTo: 'publish');
        allArtisans = await query.limit(50).get();
        ...
        // First pass: prefer artisans that match the booking category
        if (allArtisans.docs.isNotEmpty && (safeCatName.isNotEmpty || safeCatId.isNotEmpty)) {
          for (final doc in allArtisans.docs) {
            final data = doc.data();
            if (!_isPublished(data['status']) || !_isArtisanActive(data)) continue;
            final artisanCats = (data['categories'] ?? data['category'] ?? '').toString().toLowerCase();
            if ((safeCatName.isNotEmpty && artisanCats.contains(safeCatName)) ||
                (safeCatId.isNotEmpty && artisanCats.contains(safeCatId))) {
              assignedArtisanId = doc.id;
              print(
                  '[ai_artisan_fallback] Found category-matched artisan=$assignedArtisanId via relaxed search');
              break;
            }
          }
        }

        // Second pass: any active artisan (if category match failed)
        if (assignedArtisanId == null && allArtisans.docs.isNotEmpty) {
          for (final doc in allArtisans.docs) {
            final data = doc.data();
            if (_isPublished(data['status']) && _isArtisanActive(data)) {
              assignedArtisanId = doc.id;
              print(
                  '[ai_artisan_fallback] Found artisan=$assignedArtisanId via relaxed search (no category match)');
              break;
            }
          }
        }
    ...
    try {
      ...
      // For RFQ bookings, prefer the AI quote total over catalog-inferred cost.
      final double rfqAiTotal = isRFQ ? _extractAiQuoteTotal(resolvedAiQuote) : 0.0;
      final String effectiveCost = isRFQ
          ? (rfqAiTotal > 0
              ? rfqAiTotal.toStringAsFixed(2)
              : (totalCost > 0 ? totalCost.toString() : 'TBD'))
          : (totalCost > 0 ? totalCost.toString() : 'TBD');

    // Resolve a real street address when the client selected "current location"
    // so the artisan sees an actual address instead of a literal placeholder.
    String resolvedAddress = providedAddress.trim();
    if (serviceOnCurrentLocation &&
        (resolvedAddress.isEmpty ||
         resolvedAddress == 'Client current location' ||
         resolvedAddress.toLowerCase() == 'current location')) {
      try {
        final dLat = double.tryParse(userLat);
        final dLng = double.tryParse(userLng);
        if (dLat != null && dLng != null && dLat != 0 && dLng != 0) {
        ...
        debugPrint('Reverse geocode in createBookingAndNotify failed: $e');
      }
    }
    ...
    final FutureBookingModel booking = FutureBookingModel(
      ...
      userProvidedAddress: resolvedAddress,
      userLat: userLat,
      userLng: userLng,
    ...
    bookingMap['order_no'] =
        isRFQ ? '' : await generateDateBasedOrderNo();
    ...
    if (isRFQ) {
      ...
      if (clientBuysMaterials || underThreshold) {
        ...
        if (assignedArtisanIds.isNotEmpty) {
          // Resolve artisan names for admin visibility / record-keeping.
          final Map<String, String> artisanNamesById = {};
          for (final artId in assignedArtisanIds) {
            try {
              final doc = await serviceProviderRef.doc(artId).get();
              if (doc.exists) {
                final d = doc.data() ?? {};
                final name = (d['name'] ?? d['userName'] ?? d['full_name'] ?? artId).toString();
                artisanNamesById[artId] = name;
              }
            } catch (_) {}
          }
          ...
          await futureBookingsRef.doc(bookingId).update({
            ...
            'status': 'rfq_pending',
            // Store first artisan name for quick display in admin list.
            if (artisanNamesById.isNotEmpty)
              'artisan_name': artisanNamesById.values.first,
          });
