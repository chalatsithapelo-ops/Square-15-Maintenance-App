                                          ...
                                          builder: (context, snapshot) {
                                            if (!snapshot.hasData) {
                                              ...
                                              if (userDoc.isEmpty) {
                                                ...
                                                userName =
                                                    userDoc.first["name"];
                                                final clientPhone = (userDoc.first.data() as Map<String, dynamic>?)?['phone']?.toString().trim() ?? '';
                                                return Row(
                                                  children: [
                                                    Expanded(
                                                      child: Text(
                                                    ...
                                                    if (clientPhone.isNotEmpty && data.accept == "1")
                                                      IconButton(
                              ...
                              child: Column(
                                ...
                                children: [
                                      ...
                                      Align(
                                        ...
                                        child: Builder(
                                          builder: (context) {
                                            ...
                                            if (data.accept == '1' &&
                                                (payStatusLower.isEmpty ||
                                                 payStatusLower == 'pending' ||
                                                 payStatusLower == 'pending_payment')) {
                                              // Cross-check the linked futureBookings doc.
                                              // If the client already paid on the booking but
                                              // tasksManagement wasn't synced, auto-fix and
                                              // show the Go to Site button.
                                              final fbId0 =
                                                  (data.futureBookingId ?? '')
                                                      .toString()
                                                      .trim();
                                              if (fbId0.isNotEmpty) {
                                                return FutureBuilder<DocumentSnapshot>(
                                                  future: FutureBookingService
                                                      .futureBookingsRef
                                                      .doc(fbId0)
                                                      .get(),
                                                  builder: (ctx, snap) {
                                                    if (snap.hasData &&
                                                        snap.data!.exists) {
                                                      final fbData = snap.data!
                                                              .data()
                                                          as Map<String, dynamic>? ??
                                                          {};
                                                      final fbPayment =
                                                          (fbData['payment_status'] ??
                                                                  '')
                                                              .toString()
                                                              .trim()
                                                              .toLowerCase();
                                                      final fbWallet =
                                                          (fbData['wallet_deducted'] ??
                                                                  '')
                                                              .toString()
                                                              .trim()
                                                              .toLowerCase();
                                                      if (fbPayment == 'paid' ||
                                                          fbWallet == 'yes') {
                                                        // Auto-sync tasksManagement so this
                                                        // self-corrects for future loads.
                                                        final docId =
                                                            (data.id ?? '')
                                                                .toString()
                                                                .trim();
                                                  ...
                                                  SizedBox(
                                                    ...
                                                    child: ElevatedButton(
                                                      ...
                                                      onPressed: () async {
                                                        ...
                                                        // Show GPS / No GPS choice
                                                        final choice = await showDialog<String>(
                                                          context: context,
                                                          builder: (ctx) => AlertDialog(
                                                            title: const Text('Navigate to Site'),
                                                            content: const Text('How would you like to navigate?'),
                                                            actions: [
                                                              TextButton.icon(
                                                                icon: const Icon(Icons.navigation, color: Colors.blue),
                                                                label: const Text('Use GPS (Google Maps)'),
                                                                onPressed: () => Navigator.pop(ctx, 'gps'),
                                                              ),
                                                              TextButton.icon(
                                                                icon: const Icon(Icons.map_outlined, color: Colors.green),
                                                                label: const Text('No GPS (In-App Map)'),
                                                                onPressed: () => Navigator.pop(ctx, 'no_gps'),
                                                              ),
                                                            ],
                                                          ),
                                                        );
                                                        ...
                                                        if (choice == 'gps') {
                                                          // Launch external Google Maps with turn-by-turn navigation
                                                          final destLat = data.otherLat ?? '';
                                                          final destLng = data.otherLng ?? '';
                                                          if (destLat.isNotEmpty && destLng.isNotEmpty) {
                                                            final uri = Uri.parse(
                                                            ...
                                                            EasyLoading.showError('Client location coordinates not available');
                                                          }
                                                        } else {
                                                          // No GPS — artisan chose to go without navigation
                                                          EasyLoading.showSuccess(
                                                            'Status updated — heading to site without GPS',
                                  ...
                                  data.status == "progress"
                                      ...
                                      : const SizedBox(),
                                  // ── Buying Material button ──
                                  data.status == "progress" &&
                                          artisanImagesNorm == '0' &&
                                          (data.buyingMaterial ?? '') != 'true'
                                      ? Padding(
                                          ...
                                          child: SizedBox(
                                            ...
                                            child: ElevatedButton.icon(
                                              ...
                                              onPressed: () async {
                                                ...
                                                try {
                                                  await FirebaseService.tasksManagementRef.doc(docId).update({
                                                    'buying_material': 'true',
                                                    'updated_at': DateTime.now().toString(),
                                                  });
                                                  // Notify client
                                                  final clientId = (data.userId ?? '').toString().trim();
                                                  if (clientId.isNotEmpty) {
                                                    FutureBookingService.sendNotificationToUser(
                                                      userId: clientId,
                                                      title: 'Artisan Buying Material',
                                                      message: 'Your artisan is purchasing materials needed for the job.',
                                                      type: 'artisan_buying_material',
                                                      data: {
                                                        'type': 'artisan_buying_material',
                                                        'task_management_id': docId,
                                                      },
                                                    ).catchError((e) {
                                                      debugPrint('Buying Material notification error: \$e');
                                                    });
                                                  }
                                                  EasyLoading.showSuccess('Status updated: Buying Material');
                                                } catch (e) {
                                              ...
                                              child: Container(
                                                ...
                                                child: Row(
                                                  children: [
                                                    ...
                                                    Text(
                                                      'Buying Material — status sent to client',
                                                      style: GoogleFonts.lato(
