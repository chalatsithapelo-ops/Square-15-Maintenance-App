...
class _CreateFutureBookingScreenState extends State<CreateFutureBookingScreen> {
  ...
  bool isRFQ = false;
  String rfqReason = '';
  PhotoDiagnosis? _photoDiagnosis;
  bool _diagnosisRunning = false;

  final TextEditingController _taskSearchController = TextEditingController();
  String _taskSearchQuery = '';
  String _taskFilter = 'all'; // all | fixed | rfq
  List<String> _categoryScopeIds = [];
  Future<String>? _taskCategoryFieldFuture;
  Future<List<TaskModel>>? _tasksFuture;

  // Cache category scope IDs to avoid repeated queries
  static final Map<String, List<String>> _categoryScopeCache = {};

  Future<String> _detectTaskCategoryField() async {
    try {
      final snap = await FirebaseService.taskRef.limit(1).get();
      if (snap.docs.isEmpty) return 'categoryId';
      final data = (snap.docs.first.data() as Map<String, dynamic>?) ??
          <String, dynamic>{};
      if (data.containsKey('categoryId')) return 'categoryId';
      if (data.containsKey('category_id')) return 'category_id';
    } catch (_) {
      // ignore
    }
    return 'categoryId';
  ...
  Future<String> _getTaskCategoryField() {
    _taskCategoryFieldFuture ??= _detectTaskCategoryField();
    return _taskCategoryFieldFuture!;
  }

  List<List<String>> _chunkIds(List<String> ids, int chunkSize) {
    if (ids.isEmpty) return const [];
    final size = chunkSize.clamp(1, 10);
  ...
  Future<List<TaskModel>> _fetchPublishedTasksForScope() async {
    final categoryId = (selectedCategoryId ?? '').trim();
    if (categoryId.isEmpty) return [];

    final field = await _getTaskCategoryField();
    final scope =
        _categoryScopeIds.isNotEmpty ? _categoryScopeIds : [categoryId];

    // Firestore whereIn limit is 10.
    final chunks = _chunkIds(scope, 10);
    final seenIds = <String>{};
    final out = <TaskModel>[];

    for (final ids in chunks) {
      try {
        final snap = await FirebaseService.taskRef
        ...
        for (final d in snap.docs) {
          final data = d.data();
          final t = TaskModel.fromDocument(data);
          final id = (t.id ?? '').toString().trim();
          if (id.isNotEmpty) {
            if (seenIds.add(id)) out.add(t);
          } else {
            // Fallback: keep doc-based uniqueness if task has no id field.
            if (seenIds.add(d.id)) out.add(t);
          }
        }
      } catch (e) {
        debugPrint('[Tasks] Error fetching tasks chunk ($field): $e');
      }
    }
  ...
  Future<void> _loadCategoryScopeIds(String categoryId) async {
    // Check cache first
    if (_categoryScopeCache.containsKey(categoryId)) {
      if (!mounted) return;
      setState(() {
        _categoryScopeIds = _categoryScopeCache[categoryId]!;
      });
      return;
    }
    ...
    try {
      ...
      // Cache the result
      _categoryScopeCache[categoryId] = idList;

      if (!mounted) return;
      setState(() {
        _categoryScopeIds = idList;
        _tasksFuture = _fetchPublishedTasksForScope();
      });
    } catch (e) {
      debugPrint('[Category Scope] Error loading subcategories: $e');
      // Fallback to just the selected category.
      final fallback = [categoryId];
      _categoryScopeCache[categoryId] = fallback;

      if (!mounted) return;
      setState(() {
        _categoryScopeIds = fallback;
        _tasksFuture = _fetchPublishedTasksForScope();
      });
    }
  }
      ...
      body: SingleChildScrollView(
        ...
        child: Column(
          ...
          children: [
            ...
            Text(
              'Choose Booking if you want a fixed-priced service, or RFQ if you need a quotation.',
            ...
            RadioListTile<String>(
              ...
              title: Text('Booking (fixed-priced service)',
                ...
                return DropdownButtonFormField<String>(
                  ...
                  onChanged: (value) {
                    setState(() {
                      ...
                      _tasksFuture =
                          value == null ? null : _fetchPublishedTasksForScope();
                    });
                    if (value != null) {
            ...
            if (selectedCategoryId != null && !isRFQ) ...[
              ...
              FutureBuilder<List<TaskModel>>(
                future: _tasksFuture ?? _fetchPublishedTasksForScope(),
                builder: (context, taskSnapshot) {
                  if (taskSnapshot.connectionState == ConnectionState.waiting) {
                    return const Center(child: CircularProgressIndicator());
                  }

                  List<TaskModel> tasks = taskSnapshot.data ?? [];
            ...
            TextField(
              ...
              decoration: InputDecoration(
                hintText:
                    'Describe the issue in detail (what is broken, where, and any symptoms)',
                border:
            ...
            Text('Upload Work Images',
                style: GoogleFonts.roboto(
                    fontSize: 18, fontWeight: FontWeight.bold)),
            const SizedBox(height: 5),
            Text('Show us the current state of the work',
                style: GoogleFonts.roboto(fontSize: 12, color: Colors.grey)),
            const SizedBox(height: 10),
            Container(
              padding: const EdgeInsets.all(15),
            ...
            // AI Photo Diagnosis Card
            if (_diagnosisRunning)
              Container(
                ...
                child: Row(
                  children: [
                    ...
                    Text('AI is analyzing your photo...', style: GoogleFonts.roboto(fontSize: 13, color: Colors.blue.shade700)),
                  ],
                ),
              ),
            if (_photoDiagnosis != null && !_diagnosisRunning)
              Container(
                margin: const EdgeInsets.only(top: 12),
                padding: const EdgeInsets.all(12),
                decoration: BoxDecoration(
                  color: _photoDiagnosis!.severity == 'emergency' ? Colors.red.shade50
                      : _photoDiagnosis!.severity == 'high' ? Colors.orange.shade50
                      : Colors.green.shade50,
                  borderRadius: BorderRadius.circular(10),
                  border: Border.all(
                    color: _photoDiagnosis!.severity == 'emergency' ? Colors.red.shade300
                        : _photoDiagnosis!.severity == 'high' ? Colors.orange.shade300
                  ...
                  children: [
                    Row(
                      children: [
                        ...
                        Text('AI Photo Analysis', style: GoogleFonts.roboto(fontWeight: FontWeight.bold, fontSize: 14)),
                        const Spacer(),
                        Container(
                          padding: const EdgeInsets.symmetric(horizontal: 8, vertical: 2),
                          decoration: BoxDecoration(
                            color: _photoDiagnosis!.severity == 'emergency' ? Colors.red
                                : _photoDiagnosis!.severity == 'high' ? Colors.orange
                                : Colors.green,
                            borderRadius: BorderRadius.circular(10),
                          ),
                          child: Text(
                            _photoDiagnosis!.severity.toUpperCase(),
                    ...
                    const SizedBox(height: 8),
                    Text(_photoDiagnosis!.description, style: GoogleFonts.roboto(fontSize: 13)),
                    if (_photoDiagnosis!.recommendedAction.isNotEmpty) ...[
                      const SizedBox(height: 6),
                      Text('Recommended: ${_photoDiagnosis!.recommendedAction}',
                          style: GoogleFonts.roboto(fontSize: 12, color: Colors.grey.shade700, fontStyle: FontStyle.italic)),
                    ],
                  ],
                ),
              ),
  ...
  Future<void> _pickImage(ImageSource source) async {
    try {
      ...
      if (pickedFile != null) {
        ...
        // Auto-trigger AI diagnosis on first image
        if (workImages.length == 1 && _photoDiagnosis == null) {
          _runPhotoDiagnosis(file);
        }
      }
    } catch (e) {
  ...
  Future<void> _runPhotoDiagnosis(File imageFile) async {
    if (_diagnosisRunning) return;
    setState(() => _diagnosisRunning = true);
    try {
      final diagnosis = await AIPhotoDiagnosisService.instance.diagnosePhoto(
        imageFile: imageFile,
        userDescription: selectedCategoryName,
      );
      if (mounted) {
        setState(() {
          _photoDiagnosis = diagnosis;
          // Enrich description if empty and diagnosis has useful info
          if (descriptionController.text.trim().isEmpty &&
              diagnosis.confidence > 0.3 &&
              diagnosis.description.isNotEmpty) {
            descriptionController.text = diagnosis.description;
          }
        });
      }
    } catch (e) {
      debugPrint('Photo diagnosis error: $e');
    } finally {
      if (mounted) setState(() => _diagnosisRunning = false);
    }
  }
  ...
  Future<void> _createBooking() async {
    ...
    try {
      ...
      // ── Geo-fence check ──
      final checkLat = serviceOnCurrentLocation
          ? double.tryParse(appController.userLat.value)
          : double.tryParse(pickedLat);
      final checkLng = serviceOnCurrentLocation
          ? double.tryParse(appController.userLng.value)
          : double.tryParse(pickedLng);
      if (checkLat != null && checkLng != null && mounted) {
        final allowed = await ServiceAreaChecker.checkAndShowDialog(
          context: context,
          lat: checkLat,
          lng: checkLng,
        );
      ...
      final result = await FutureBookingService.createBookingAndNotify(
        ...
        createdBy: 'manual',
        aiDiagnosis: _photoDiagnosis?.toMap(),
      );
