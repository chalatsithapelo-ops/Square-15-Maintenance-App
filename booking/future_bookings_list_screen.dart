...
class _FutureBookingsListScreenState extends State<FutureBookingsListScreen> {
  ...
  Widget build(BuildContext context) {
    ...
    final docs = allDocs.where((d) {
      ...
      // Future tab should not show clearly past-dated bookings (including RFQs).
      final scheduled =
          FutureBookingService.tryParseScheduledDateTimeFromDocument(data);
      if (scheduled != null && scheduled.isBefore(todayStart)) return false;
      ...
      if (!isRfq && scheduled == null) return false;
      if (isRfq) return true;

      if (allowedStatuses.contains(status)) return true;
      if (paymentStatus == 'paid') return true;
      if (walletDeducted == 'yes') return true;
      if (artisanConfirmed == 'yes') return true;
      if (tasksManagementId.isNotEmpty) return true;
